...
var CSSReader_element_cssDefinition

var CSSReader_container

var CSSReader_current_element

// CSS Properties
var CSSReader_pFont = new Array(
    'font-family',
    'font-size',
    'font-style',
    'font-variant',
...
var CSSReader_pColorBg = new Array(
...
var CSSReader_pPositioning = new Array(
    'position',
    'top',
    'bottom',
    'right',
...
var CSSReader_pList = new Array(
    'list-style-image',
    'list-style-type',
    'list-style-position'
);

var CSSReader_pTable = new Array(
    'border-collapse',
    'border-spacing',
    'caption-side',
    'empty-cells',
...
var CSSReader_pMisc = new Array(
    'overflow',
    'cursor',
    'visibility'
);

var CSSReader_pEffect = new Array(
    'transform',
    'transition',
    'outline',
    'outline-offset',
...
var CSSReader_categories = {
    'pFontText': CSSReader_pFont,
    'pColorBg': CSSReader_pColorBg,
    'pBox': CSSReader_pBox,
    'pPositioning': CSSReader_pPositioning,
    'pList': CSSReader_pList,
    'pTable': CSSReader_pTable,
    'pMisc': CSSReader_pMisc,
    'pEffect': CSSReader_pEffect
};

var CSSReader_categoriesTitle = {
    'pFontText': 'Font & Text',
...
// Table tagnames
var CSSReader_tableTagNames = new Array(
    'TABLE',
    'CAPTION',
    'THEAD',
    'TBODY',
...
var CSSReader_listTagNames = new Array(
    'UL',
    'LI',
    'DD',
    'DT',
...
function DecToHex(nb) {
    ...
    nbHexa += CSSReader_hexa[Math.floor(nb / 16)];
    nb = nb % 16;
    nbHexa += CSSReader_hexa[nb];
...
function SetCSSProperty(element, property) {
    ...
    var li = document.getElementById('CSSReader_' + property);
...
function HideCSSCategory(category) {
    var document = GetCurrentDocument();
    var div = document.getElementById('CSSReader_' + category);
...
function UpdateTable(element, tagName) {
    if (IsInArray(CSSReader_tableTagNames, tagName)) {
        var nbProperties = 0;
...
function UpdateList(element, tagName) {
    if (IsInArray(CSSReader_listTagNames, tagName)) {
...
function CSSReaderMouseOver(e) {
    ...
    CSSReader_element = this;

    cssReaderRemoveElement("cssReaderInsertMessage");

    e.stopPropagation();

    // generate simple css definition
    CSSReader_element_cssDefinition = this.tagName.toLowerCase() + (this.id == '' ? '' : ' #' + this.id) + (this.className == '' ? '' : ' .' + this.className) + " {\n";

    CSSReader_element_cssDefinition += "\t/* Font & Text */\n";
    for (var i = 0; i < CSSReader_pFont.length; i++)
        CSSReader_element_cssDefinition += "\t" + CSSReader_pFont[i] + ': ' + element.getPropertyValue(CSSReader_pFont[i]) + ";\n";

    CSSReader_element_cssDefinition += "\n\t/* Color & Background */\n";
    for (var i = 0; i < CSSReader_pColorBg.length; i++)
        CSSReader_element_cssDefinition += "\t" + CSSReader_pColorBg[i] + ': ' + element.getPropertyValue(CSSReader_pColorBg[i]) + ";\n";

    CSSReader_element_cssDefinition += "\n\t/* Box */\n";
    for (var i = 0; i < CSSReader_pBox.length; i++)
        CSSReader_element_cssDefinition += "\t" + CSSReader_pBox[i] + ': ' + element.getPropertyValue(CSSReader_pBox[i]) + ";\n";

    CSSReader_element_cssDefinition += "\n\t/* Positioning */\n";
    for (var i = 0; i < CSSReader_pPositioning.length; i++)
        CSSReader_element_cssDefinition += "\t" + CSSReader_pPositioning[i] + ': ' + element.getPropertyValue(CSSReader_pPositioning[i]) + ";\n";

    CSSReader_element_cssDefinition += "\n\t/* List */\n";
    for (var i = 0; i < CSSReader_pList.length; i++)
        CSSReader_element_cssDefinition += "\t" + CSSReader_pList[i] + ': ' + element.getPropertyValue(CSSReader_pList[i]) + ";\n";

    CSSReader_element_cssDefinition += "\n\t/* Table */\n";
    for (var i = 0; i < CSSReader_pTable.length; i++)
        CSSReader_element_cssDefinition += "\t" + CSSReader_pTable[i] + ': ' + element.getPropertyValue(CSSReader_pTable[i]) + ";\n";

    CSSReader_element_cssDefinition += "\n\t/* Miscellaneous */\n";
    for (var i = 0; i < CSSReader_pMisc.length; i++)
        CSSReader_element_cssDefinition += "\t" + CSSReader_pMisc[i] + ': ' + element.getPropertyValue(CSSReader_pMisc[i]) + ";\n";

    CSSReader_element_cssDefinition += "\n\t/* Effects */\n";
    for (var i = 0; i < CSSReader_pEffect.length; i++)
        CSSReader_element_cssDefinition += "\t" + CSSReader_pEffect[i] + ': ' + element.getPropertyValue(CSSReader_pEffect[i]) + ";\n";
...
function CSSReaderMouseOut(e) {
    this.style.outline = '';

    e.stopPropagation();
}

function CSSReaderMouseMove(e) {
    var document = GetCurrentDocument();
    var block = document.getElementById('CSSReader_block');
    ...
    inView = CSSReaderIsElementInViewport(block);
...
function CSSReaderIsElementInViewport(el) {
...
function CSSReader() {
    ...
    this.CreateBlock = function () {
        ...
        if (document) {
            ...
            center.id = 'CSSReader_center';

            for (var cat in CSSReader_categories) {
                var div = document.createElement('div');

                div.id = 'CSSReader_' + cat;
                div.className = 'CSSReader_category';

                var h2 = document.createElement('h2');

                h2.appendChild(document.createTextNode(CSSReader_categoriesTitle[cat]));

                var ul = document.createElement('ul');
                var properties = CSSReader_categories[cat];

                for (var i = 0; i < properties.length; i++) {
                    var li = document.createElement('li');

                    li.id = 'CSSReader_' + properties[i];

                    var spanName = document.createElement('span');

                    spanName.className = 'CSSReader_property';
        ...
        cssReaderInsertMessage("CSSReader loaded! Hover any element you want to inspect in the page.");
    ...
    this.AddEventListeners = function () {
        ...
        for (var i = 0; i < elements.length; i++) {
            elements[i].addEventListener("mouseover", CSSReaderMouseOver, false);
            elements[i].addEventListener("mouseout", CSSReaderMouseOut, false);
            elements[i].addEventListener("mousemove", CSSReaderMouseMove, false);
        }
    ...
    this.RemoveEventListeners = function () {
        ...
        for (var i = 0; i < elements.length; i++) {
            elements[i].removeEventListener("mouseover", CSSReaderMouseOver, false);
            elements[i].removeEventListener("mouseout", CSSReaderMouseOut, false);
            elements[i].removeEventListener("mousemove", CSSReaderMouseMove, false);
        }
        this.haveEventListeners = false;
    }
...
/*
* Check if CSSReader is enabled
*/
CSSReader.prototype.IsEnabled = function () {
    var document = GetCurrentDocument();

    if (document.getElementById('CSSReader_block')) {
        return true;
    }
...
* Enable CSSReader
*/
CSSReader.prototype.Enable = function () {
    console.log("viewe enabled");
    var document = GetCurrentDocument();
    var block = document.getElementById('CSSReader_block');
...
* Freeze CSSReader
*/
CSSReader.prototype.Freeze = function () {
    var document = GetCurrentDocument();
    var block = document.getElementById('CSSReader_block');
...
* Unfreeze CSSReader
*/
CSSReader.prototype.Unfreeze = function () {
    var document = GetCurrentDocument();
    var block = document.getElementById('CSSReader_block');
    if (block && !this.haveEventListeners) {
...
*/
function cssReaderInsertMessage(msg) {
    ...
    // https://github.com/miled/cssReader/issues/5
    // https://github.com/miled/cssReader/issues/6
    // var beforeMe = document.getElementsByTagName("body");
    // document.body.insertBefore( oNewP, beforeMe[0] );

    // https://github.com/zchee/cssReader/commit/dad107d27e94aabeb6e11b935ad28c4ff251f895
...
*/
function cssReaderRemoveElement(divid) {
...
chrome.runtime.onMessage.addListener((request) => {
    cssReaderCopyCssToConsole(request.args)
...
*/
function cssReaderCopyCssToConsole(type) {
    if ('el' == type) return console.log(CSSReader_element);
    if ('id' == type) return console.log(CSSReader_element.id);
    if ('tagName' == type) return console.log(CSSReader_element.tagName);
    if ('className' == type) return console.log(CSSReader_element.className);
    if ('style' == type) return console.log(CSSReader_element.style);
    if ('cssText' == type) return console.log(document.defaultView.getComputedStyle(CSSReader_element, null).cssText);
    if ('getComputedStyle' == type) return console.log(document.defaultView.getComputedStyle(CSSReader_element, null));
    if ('simpleCssDefinition' == type) return console.log(CSSReader_element_cssDefinition);
}

/*
*  Close css Reader on clicking 'esc' key
*  Freeze css Reader on clicking 'f' key
*/
function CssReaderKeyMap(e) {
    if (!cssReader.IsEnabled())
        return;
    ...
    // f: Freeze or Unfreeze the css Reader if the cssReader is enabled
    if (e.keyCode === 70) {
        if (cssReader.haveEventListeners) {
            cssReader.Freeze();
        }
        else {
            cssReader.Unfreeze();
    ...
    if (e.keyCode === 67) {
        window.prompt("Simple Css Definition :\n\nYou may copy the code below then hit escape to continue.", CSSReader_element_cssDefinition);
    }
}


/*
* CSSReader entry-point
*/
